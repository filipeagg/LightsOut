...
failure_this_prevents: a client asked what menu options ACMEPRODUCT has and answered from a project's source code, with knowledge/acme/acmeproduct/producto/Estructura_Menu_ACMEPRODUCT.md one call away.
...
may: a base whose documents live anywhere **inside** `knowledge/` — `knowledge/mercado` and `knowledge/acmecorp/mercado` alike. Nesting is not a reason to refuse.
