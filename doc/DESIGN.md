...
|---|---|---|---|
| `python3 -c "…os.environ.get('LO_VAULT_ACMEPRODUCT_PASSWORD')… print('present')"` | `credentials` | `script_exec` | `PASSWORD` inside a variable *name*. The agent was checking its own wiring; it printed `present`, never a value. And `credentials` is on the hard floor, so neither the judge nor a learned allow could rescue it — the run stopped dead. |
...
```
pwd && rg --files | sort && if [ -n "${LO_VAULT_ACMEPRODUCT_PASSWORD:-}" ]; then echo 'present'; else echo 'missing'; fi
```
...
|---|---|---|
| 1 | `python3 -c "…os.environ.get('LO_VAULT_ACMEPRODUCT_PASSWORD')… print('present')"` | a variable *name* is not a value (§7.1b) |
| 2 | `if [ -n "${LO_VAULT_ACMEPRODUCT_PASSWORD:-}" ]; then echo present; fi` | `stripPresenceTests()` (§7.1b) |
| 3 | `grep -rqF "$LO_VAULT_ACMEPRODUCT_PASSWORD" .` | — none. This is the one that opened D-1 on `acmeproduct-crop-map-prototype`. |
...
`vault_foreign` and is unreachable again — sending the ACMEPRODUCT password to a host that is not ACMEPRODUCT
is exactly the thing this class exists to stop.

### 7.1e A write the classifier cannot see is a write it must deny

`contract-prober` — Codex — could not write a single file on `acmeproduct-crop-map-prototype`. The audit
...
`contract-prober` could not write a single file on `acmeproduct-crop-map-prototype`. It said so plainly —
...
The script opens no such file. It reads `process.env.LO_VAULT_ACMEPRODUCT_USUARIO`, and the pattern
...
What still stops, and should: `console.log(process.env.LO_VAULT_ACMEPRODUCT_PASSWORD)`. Owning the key
...
Running the user's own goal end to end — "download the crops of company 42 from ACMEPRODUCT and save
...
```
R=…/src/acmeproduct_django-master; find $R/throttling $R/user -maxdepth 1 -name '*.py' | xargs wc -l
```
...
`/workspace/sources/acmeproduct_django-master`. Every attempt to reach it — `ls -la /workspace/sources/`,
...
`doc/acmeproduct_prototipo.html`. `detectPreview()` looks where code goes; the agent had put it where
...
the folder — and a linked base was never writable. So `knowledge/acme/mercado` could not be
...
```
# ANALYSIS :: curacionapi-acmeproduct
meta.doc: ANALYSIS
...
## facts
f.1.claim: base acmeproduct has 38 undocumented fields
f.1.source: knowledge:acmeproduct/tecnico/api.md
f.1.kind: preference
