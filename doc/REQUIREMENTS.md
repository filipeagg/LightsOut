...
- PE-10 MUST. The system learns from the gates a human answers. When a person allows a permission doubt whose class was `other` — a command the classifier did not recognise — the **shape** of that command is remembered, and the same shape never opens a gate again. Only `other`: a class the classifier understood is the packs' business, and the hard floor of PE-03 is untouched. Learned allows are listed, counted and revocable from both surfaces; a shape used often is a matcher somebody should write. A chain must not stop for eleven minutes over `find … | xargs wc -l` twice.
