...
- TP-04 MUST. Templates are created, cloned and edited from the panel (WP-02): add, remove and reorder phases, pick the agent per phase, edit the phase instructions. The panel writes YAML into `templates/`; builtins are cloned rather than modified, and a workspace file shadowing a builtin id wins.
