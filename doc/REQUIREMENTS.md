...
- PE-09 MUST. A project may declare **read-only areas** of the workspace outside its own directory — the imported source of the system it is analysing, a folder of material the user dropped in. A path inside a declared area classifies as a read of the project rather than as `outside_workspace`, so an agent can read it and copy from it into its project without a human gate. Writing into an area is never allowed. An area is declared per project, from the panel and from MCP, is recorded with who declared it, and can never be: the workspace root, `agents/`, `templates/`, `vault.yaml`, anything under `knowledge/` (KB-05 governs that), or another project's directory. Without a declared area the behaviour is unchanged: outside the project is denied.
...
- KB-09 MUST. A base holds a tree, not a flat list: documents are found in subfolders too, and each one is identified and labelled by its path inside the base (`acmeproduct/tecnico/api.md`), because that path is usually how the person organising the folder said what the document is about.
