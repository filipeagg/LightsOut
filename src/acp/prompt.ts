  ...
  if (input.writableKnowledge) {
    ...
    blocks.push(
      [
        ...
        "",
        "**Your structured edit tool will refuse this path.** `apply_patch`, `Edit` and `Write` scope",
        "themselves to the directory the session started in, which is your project, and they refuse",
        "anything outside it before LightsOut is asked. That refusal is the tool's, not a policy",
        "denial: the policy allows this write. Use the shell instead, which is not scoped that way:",
        "",
        "  mkdir -p <base.dir>",
        "  cat > <base.dir>/index.md <<'MD'",
        "  …the document…",
        "  MD",
        "",
        "Python works for the same reason. Do not spend a turn arguing with the edit tool, and do not",
        "conclude the base is read-only: write it with the shell and carry on.",
      ].join("\n"),
    );
  }
