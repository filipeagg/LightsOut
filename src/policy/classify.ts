...
export type ClassifyInput = {
  ...
  /**
   * Environment variable names of the vault entries **this run resolved** — `LO_VAULT_ACMEPRODUCT_
 ...
 * were the entire case — `grep -rqF "$LO_VAULT_ACMEPRODUCT_PASSWORD" .` and every future spelling of
 ...
 * not the entry's own host is `vault_foreign` — sending the ACMEPRODUCT password anywhere but ACMEPRODUCT is
 ...
 * explicitly asked for, was ruled exfiltration — while `https://api.example.com`, the
...
export const DEFAULT_MATCHERS: Record<string, string[]> = {
  ...
  credentials: [
    ...
    // `os.environ.get('LO_VAULT_ACMEPRODUCT_PASSWORD')` is an agent checking its own wiring, and
 ...
 * string with the word in it. What it actually caught, on `acmeproduct-mapa-cultivos`:
 *
...
const SCRIPT_BODY_FAMILIES: [ActionClass, RegExp, string][] = [
  ...
  // ACMEPRODUCT_PASSWORD')` followed by "present"/"missing" is how an agent checks its own wiring
 ...
 *
 * `if [ -n "${LO_VAULT_ACMEPRODUCT_PASSWORD:-}" ]; then echo present; else echo missing; fi` expands the
