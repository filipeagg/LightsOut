 ...
 * (`/workspace/sources/acmeproduct_django-master`), denied every way of reading it, writing reports
 * about being blocked.
 */
...
beforeEach(async () => {
  ...
  await mkdir(path.join(workspace, "sources", "acmeproduct_django-master"), { recursive: true });
  await mkdir(path.join(workspace, "projects", "other"), { recursive: true });
  await mkdir(path.join(workspace, "knowledge", "acmeproduct"), { recursive: true });
...
describe("what may be declared an area", () => {
  it("accepts a directory of the workspace, relative or absolute", () => {
    expect(validateArea(workspace, project, "sources/acmeproduct_django-master").relative).toBe(
      "sources/acmeproduct_django-master",
    );
    expect(
  ...
  it("refuses what would widen the boundary too far, each with its reason", () => {
    ...
    expect(() => validateArea(workspace, project, "knowledge/acmeproduct")).toThrow(/knowledge base/);
  ...
  it("accepts a single file, because the material is sometimes one archive", async () => {
    ...
    await writeFile(path.join(workspace, "sources", "acmeproduct.zip"), "zip", "utf8");
    expect(validateArea(workspace, project, "sources/acmeproduct.zip").relative).toBe(
      "sources/acmeproduct.zip",
    );
  });

  it("is stored once per project and path", () => {
    repos.projects.create({ id: "curation", name: "Curation", path: project, context: "goal: x" });
    repos.areas.add({ projectId: "curation", path: "sources/acmeproduct_django-master", addedBy: "mcp" });
    repos.areas.add({ projectId: "curation", path: "sources/acmeproduct_django-master", addedBy: "mcp" });
    expect(repos.areas.list("curation")).toHaveLength(1);
    expect(repos.areas.remove("curation", "sources/acmeproduct_django-master")?.path).toBe(
      "sources/acmeproduct_django-master",
    );
...
describe("classification with an area (the case that failed)", () => {
  const area = () => [path.join(workspace, "sources", "acmeproduct_django-master")];
  ...
  it("turns the denied listing into a read", () => {
    const target = path.join(workspace, "sources", "acmeproduct_django-master");
  ...
  it("allows copying out of the area into the project", () => {
    const target = path.join(workspace, "sources", "acmeproduct_django-master");
    const decision = classify(`cp -r ${target} ./sources/acmeproduct_django-master`);
  ...
  it("changes nothing outside the declared directory", () => {
    ...
    expect(classify(`ls -la ${workspace}/sources/acmeproduct_django-master`, []).class).toBe(
      "outside_workspace",
    );
  });
...
describe("an area may be writable, and only where it was declared so (PE-09 amended)", () => {
  const shared = () => path.join(workspace, "sources", "acmeproduct_django-master");
  ...
  it("stores the access and changes it when the same path is declared again", () => {
    ...
    const first = repos.areas.add({
      projectId: "curation",
      path: "sources/acmeproduct_django-master",
      addedBy: "panel",
    });
