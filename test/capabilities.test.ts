...
const PROJECT = "/workspace/projects/acmeproduct";
...
describe("the three commands that stopped a real run", () => {
  ...
  it("checking whether a vault variable is set is not reading a secret", () => {
    expect(
      classify(
        `python3 -c "import os; print('present' if os.environ.get('LO_VAULT_ACMEPRODUCT_PASSWORD') else 'missing_or_empty')"`,
      ),
    ).toBe("script_exec");
    expect(classify(`python3 -c "import os; print(os.environ.get('LO_VAULT_ACMEPRODUCT_USUARIO'))"`)).toBe(
      "credentials",
    );
  });
  ...
  it("testing a vault variable in the shell is not reading a secret either", () => {
    ...
    expect(
      classify(
        `pwd && rg --files | sort && if [ -n "\${LO_VAULT_ACMEPRODUCT_PASSWORD:-}" ]; then echo 'vault.password=present'; else echo 'vault.password=missing'; fi`,
      ),
    ...
    expect(classify(`[ -z "$LO_VAULT_ACMEPRODUCT_USUARIO" ] && echo missing`)).toBe("project_read");
...
describe("a vault entry implies its own host (VT-07)", () => {
  it("takes the hosts from the entries the run resolved", () => {
    expect(
      vaultHosts({
        index: [
          { id: "acmeproduct", label: "ACMEPRODUCT", base_url: "https://api.acmeproduct.example/v2", auth: "basic", fields: [], test_only: true },
    ...
    ).toEqual(["api.acmeproduct.example"]);
  });
});
