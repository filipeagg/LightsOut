...
describe("the manual served over MCP (MC-09)", () => {
  ...
  it("names the curated bases an install actually has (KB-01, §10.0d)", () => {
    ...
    const two = buildServerInstructions([base("acmeproduct", "technical"), base("mercado", "market")]);
    // The failure this closes: a client read "runs coding agents" and answered a question about
    // ACMEPRODUCT from a project's code, with the acmeproduct base one call away.
    expect(two).toMatch(/acmeproduct \(technical\)/);
