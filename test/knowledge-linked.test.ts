 ...
 * may not. `knowledge/acme/mercado` being uncurateable was the failure that moved this.
 */
describe("which base owns its documents (KB-05b)", () => {
  ...
  it("owns a base that reads from deeper inside knowledge/", async () => {
    await write("knowledge/acme/mercado/index.md", "# mercado");
    const loader = await loaderWith("knowledge/acme/mercado");
    expect(loader.getOrThrow("mercado").docsDir).toBe(
      path.resolve(workspace, "knowledge/acme/mercado"),
    );
    expect(loader.ownsItsDocuments("mercado")).toBe(true);
  });
...
describe("what the curator is told about the base (KB-05c)", () => {
  ...
  it("names the absolute directory, and the trap", () => {
    const text = prompt({ baseId: "acme-mercado", dir: "/workspace/knowledge/acme-mercado" });
    expect(text).toContain("base.dir: /workspace/knowledge/acme-mercado");
    expect(text).toContain("/workspace/knowledge/acme-mercado/index.md");
