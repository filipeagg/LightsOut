 ...
 * unattended" and administration verbs, and answered questions about ACMEPRODUCT and about market news
...
beforeEach(async () => {
  ...
  await write(
    "knowledge/acmeproduct/knowledge.yaml",
    "id: acmeproduct\nname: ACMEPRODUCT\nkind: technical\ndescription: ACMEPRODUCT product and API\n",
  );
  await write(
    "knowledge/acmeproduct/producto/menu.md",
    "# Menu\n\nsections: Parcelario, Tareas, Almacén\nnote: el Vademécum vive bajo Maestros\n",
  );
  await write("knowledge/acmeproduct/tecnico/api.md", "auth: bearer token\nbase_url: /api\n");
  await write(
...
describe("searching curated knowledge (KB-13)", () => {
  it("finds the document that mentions the phrase, labelled by base and file", async () => {
    ...
    expect(hits[0]).toMatchObject({ baseId: "acmeproduct", kind: "technical", file: "producto/menu.md" });
  ...
  it("searches every base unless told otherwise", async () => {
    ...
    expect(await loader.search("precios", { baseId: "acmeproduct" })).toEqual([]);
  });
  ...
  it("refuses an unknown base by name, with the ones that exist", async () => {
    await expect(loader.search("x", { baseId: "nope" })).rejects.toThrow(/acmeproduct, mercado/);
  });
...
describe("the tool list says knowledge answers questions (KB-13)", () => {
  it("names the bases this install has in every knowledge tool's description", () => {
    ...
    for (const name of ["list_knowledge", "read_knowledge", "search_knowledge"]) {
      ...
      expect(text, `${name} does not name the bases`).toContain("acmeproduct (technical)");
