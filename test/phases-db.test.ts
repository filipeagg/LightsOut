...
describe("project knowledge repo", () => {
  it("attaches, re-attaches and detaches a base", () => {
    const projectId = seedProject();
    repos.projectKnowledge.attach({ projectId, baseId: "acmeerp", kind: "technical" });
    ...
    expect(repos.projectKnowledge.list(projectId)).toHaveLength(1);
    expect(repos.projectKnowledge.writableBase(projectId)).toBe("acmeerp");

    expect(repos.projectKnowledge.detach(projectId, "acmeerp")).toBe(true);
...
describe("vault audit repo", () => {
  it("records field names without values", () => {
    ...
    repos.vaultAudit.record(run.id, "acmeerp-sandbox", ["token", "user"]);
