...
describe("knowledge loader and injection", () => {
  ...
  it("refuses to read a document outside the base", async () => {
    await seedBase("acmeerp", "name: E\nkind: technical\n", { "a.md": "A" });
    const loader = new KnowledgeLoader(workspace);
    await loader.load();
    await expect(loader.readDocument("acmeerp", "../../secret")).rejects.toThrow(/escapes/);
  });

  it("always injects manifests and index.md, and lists what the budget left out (KB-06)", async () => {
    await seedBase("acmeerp", "name: Acmeerp\nkind: technical\ntags: [erp]\n", {
    ...
    const generous = await buildKnowledgeBlock(loader, ["acmeerp"], { budgetChars: 100000 });
    expect(generous.included.map((d) => d.file).sort()).toEqual(["big.md", "small.md"]);
    expect(generous.text).toContain("--- knowledge: acmeerp (technical) — small.md ---");
    expect(generous.omitted).toEqual([]);

    const tight = await buildKnowledgeBlock(loader, ["acmeerp"], { budgetChars: 1200 });
    ...
    expect(tight.text).toContain('read_knowledge("acmeerp", "big.md")');
  });
...
describe("vault", () => {
  it("stores values, returns views without them, and merges on update (VT-02, VT-03)", async () => {
    ...
    await vault.put("acmeerp-sandbox", {
      label: "Acmeerp sandbox",
    ...
    await vault.put("acmeerp-sandbox", { fields: { user: null } });
    const after = await vault.readAll();
    expect(after[0]?.fields).toEqual({ token: "s3cret" });
    expect(after[0]?.label).toBe("Acmeerp sandbox");
  });
