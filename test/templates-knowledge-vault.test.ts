...
describe("vault", () => {
  ...
  it("derives the id from the label, and makes it unique (VT-08)", async () => {
    const vault = new Vault(workspace);
    expect(await vault.idForLabel("ACMEPRODUCT back")).toBe("acmeproduct-back");
    await vault.put(await vault.idForLabel("ACMEPRODUCT back"), { label: "ACMEPRODUCT back" });
    // A second entry with the same label does not silently overwrite the first.
    expect(await vault.idForLabel("ACMEPRODUCT back")).toBe("acmeproduct-back-2");
  });
